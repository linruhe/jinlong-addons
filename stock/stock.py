...
class stock_move(osv.osv):
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        move_qty = {}
        for move in self.browse(cr, uid, ids, context=context):
            move_qty[move.id] = round(move.product_qty, 2)
            for link in move.linked_move_operation_ids:
                operations.add(link.operation_id)
        ...
        for ops in operations:
            ...
            main_domain = [('qty', '>', 0)]
            link_id = self.pool.get('stock.move.operation.link').search(cr, uid, [('operation_id', '=', ops.id)], context=context)
            for record in ops.linked_move_operation_ids:
                move = record.move_id
                ...
                # Handle pack in pack
                    ...
                    if record.operation_id :
                        t=record.operation_id
                    move_qty[move.id] = round((move_qty[move.id]-record.qty),2)
